...
- Triggered by Git pre-commit hook (`hooks/pre-commit`, version-controlled)
...
└── hooks/pre-commit              # Auto-build trigger (activated by npm install)
```
...
| `hooks/pre-commit` | Auto-build trigger | Runs build-posts.js on commit if .md files changed, stages generated HTML/posts.json/sitemap.xml/llms.txt. Version-controlled; the `prepare` script in package.json activates it via `core.hooksPath` on `npm install` |
