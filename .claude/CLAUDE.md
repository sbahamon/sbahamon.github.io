...
└── hooks/pre-commit              # Auto-build trigger (needs core.hooksPath)
```
...
| `hooks/pre-commit` | Auto-build trigger | Runs build-posts.js on commit if .md files changed, stages generated HTML/posts.json/sitemap.xml/llms.txt. Version-controlled; each clone must run `git config core.hooksPath hooks` once to activate it |
